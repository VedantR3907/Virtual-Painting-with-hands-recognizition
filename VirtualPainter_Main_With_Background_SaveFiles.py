...
while True:
        ...
        if fingers[1] and fingers[2]:   #checking for index and middle finger
            ...
            if 1349<x1<1540:              #checking for selecting Shape
                if 1400<x1<1500 and 50<y1<200:
                    ...
                    shape = List_s[4]
                    shape_name = 'Triangle'
            cv.rectangle(background, (x1,y1-25), (x2,y2+25), drawColor, cv.FILLED)
    ...
    #4. Checking if not index and middle finger (DRAWING MODE)
        ...
        if fingers[1] and fingers[2] == False:
            flag = 0
            cv.circle(background, (x1,y1), 15, drawColor, cv.FILLED)
            ...
            if drawColor == (0,0,0):
                ...
                cv.line(background, (xp,yp), (x1,y1), drawColor, eraserThickness)
                ...
                if shape_name == 'Freestyle':
                    cv.line(background, (xp,yp), (x1,y1), drawColor, brushThickness)       #for drawing lines using ur finger tips
                    ...
                    cv.circle(background, (x1,y1), size, drawColor, 20)
                    ...
                    cv.rectangle(background, (xt,yt), (x1,y1), drawColor, 15)
                    ...
                    cv.line(background, (xt,yt), (x1,y1), drawColor,brushThickness)
                    cv.line(background, (x1,y1) ,(xq,yq), drawColor, brushThickness)
                    cv.line(background, (xq,yq), (xt,yt), drawColor, brushThickness)
    ...
    if back_img != 0:
        background[0:720,0:191] = pic
        background[0:720,1349:1540] = shape

        background = cv.bitwise_and(background,imginv)
        background = cv.bitwise_or(background,imgCanvas)

        if(pressedKey == ord('a')):
            capture_screenshot('E:/Output_images/', background)
        ...
        if(pressedKey == ord('a')):
            capture_screenshot('E:/Output_images/', img)
        cv.imshow("Video", img)
